...
### שלב 0.5 — שאלות מוכנות לפני החקירה

אחרי שהמשתמש נותן שם עסק / URL — **לפני** שיוצאים לחקור באינטרנט**, שאל את המשתמש שאלות ממוקדות.**

בנה שאלות אחת מרוכזת עם שני שכבות:

**שכבת חובה — תמיד שאל:**
```
פרטי קשר:
□ טלפון / וואצאפ (חובה לכפתור ה-CTA)
□ כתובת / עיר שירות (אם אין — לא מוסיפים כתובת בדף)
```

**שכבת עסק — בחר 2-3 שאלות שמתאימות לתחום:**

| תחום | שאלות מומלצות |
|------|--------------|
| רפואה / רווחה | קהל יעד, וותק ניסיון, התמחויות / הסמכה, מחיר טיפול / פגישה |
| עסקים / B2B | סוגי לקוחות, סוגי פרויקטים, גיאוגרפיה, מחירון או שירות בקשרתהרות |
| חינוך / קורסים | נושאים עיקריים, פורמט (מקוון / אונליין / היבריד), מחיר, קהל יעד |
| מסעדה / אוכל | סוג מטבח, התמחויות (כשרות/טבעוני/גלוטן), דליברי או ישיבה בלבד, שעות פעילות |
| שיפוץ / בנייה | אזורי שירות, סוגי עבודה, וותק, אישי קשר / צוות |
| פייננס / ביטוח | סוג שירות, קהל יעד (עסקים/פרטיים), שאלה אחת ספציפית לתחום |
| אופנוע / אונליין | פלטפורמה (שופיפאי/אמזון/אתר עצמאי), סוג מוצר, מדיניות משלוח, מידע חיוני / חזרות |

> **כלל:** שאלת אחת מרוכזת בסגנון ישראלי וידודותי. לא שאלות פורמליות. בסוף שאלות אחת:
> *"אלו השאלות שיעזרו לי ליצור דף מדויק לעסק שלך. אחרי שתענה אצא לחקור ביסודית את השאר."*

**דוגמא לפדיקור רפואי:**
> "סבטב, כמה שאלות מהירות לפני שאתחיל:
> פרטי קשר: מה הטלפון / וואצאפ שלך? יש כתובת פיזית או שירות מרחוק?
> על העסק: כמה שנות אתה/את בתחום? ומה המקצועיות / הסמכה שיש לך?"

**מה לעשות עם התשובות:**

| אין כתובת | אין סקשיי בדף על אזור גיאוגרפי |
|----------|----------|
| יש פלאפון אבל לא וואצאפ | כפתור CTA רק לטלפון |
| יש גם וואצאפ | כפתור וואצאפ + כפתור טלפון |
